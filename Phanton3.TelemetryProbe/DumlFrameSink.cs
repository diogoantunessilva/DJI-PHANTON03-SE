...
internal sealed class DumlFrameSink : IFrameSink
{
    ...
    public async Task<bool> WriteFrameAsync(DumlFrame frame, DateTimeOffset timestamp)
    {
        try
        {
            ...
            if (_showFrames)
            {
                Console.WriteLine($"{timestamp:O} | source={source.Name} | len={frame.TotalLength} | src={Hex(frame.Sender)} | dst={Hex(frame.Receiver)} | seq={frame.Sequence} | flags={Hex(frame.Flags)} | cmdSet={Hex(frame.CommandSet)} | cmdId={Hex(frame.CommandId)} | payloadLen={frame.PayloadLength} | CRC8={Hex(frame.HeaderCrc8)} CRC8_OK={frame.Crc8Ok.ToString().ToLowerInvariant()} | CRC16=0x{frame.ReceivedCrc16:X4} CRC16_OK={frame.Crc16Ok.ToString().ToLowerInvariant()}{(semantic is null ? "" : " | " + semantic.DisplayText)}");
            }
